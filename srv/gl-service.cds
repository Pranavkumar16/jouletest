...
service GLAssistService {
  ...
  @Core.Description: 'Use this function to find the correct GL account for a specific posting, invoice, expense, or purchase. Trigger phrases: Which GL account should I use for [description] / Post [amount] to [vendor] for [description] / Book an expense for [description] / What account for [description] / I need to post [description] / GL for [description]. Returns the best-matching rule with confidence score, or an AI-suggested new rule if confidence is low. Do NOT use this to list existing rules (use listAllRules) or to explain past postings (use explainLastPosting).'
  function determineGL(
    ...
    amount      : Decimal @Core.Description: 'The posting amount in local currency as a number, for example 1200.50. Optional but helps with rule matching.',
  ...
  @Core.Description: 'Use this function to explain why the most recent posting failed, went to suspense, or what happened with the last GL determination. Trigger phrases: Why did that fail / Why did it go to suspense / Explain the last posting / What happened with that invoice / Why no match / Tell me why. Returns a natural language explanation of the most recent Posting including which rules were considered and why they did or did not match. Do NOT use this for general questions about missing rules (use listMissingRules).'
  function explainLastPosting() returns String;

  @Core.Description: 'Use this function to show gaps in the GL rule table, pending AI suggestions awaiting review, or what rules need to be added. Trigger phrases: What rules are missing / Show pending suggestions / What gaps are in the rule table / Rules that need review / What does the AI recommend / Show me the AI suggestions. Returns a grouped list of AI-proposed rules with counts of how many postings each would cover and sample descriptions. On first call scans all unmatched postings and generates AI suggestions via GPT-4.1. Do NOT use this to list existing active rules (use listAllRules).'
  ...
  @Core.Description: 'Use this action to approve a specific AI-suggested rule and add it to the active rule table. Always confirm with the user before calling. Trigger phrases: Approve the [description] rule / Add that suggestion / Yes approve it / Accept the AI recommendation. Promotes the suggestion into the GLRules table and marks the UnmatchedPosting as approved.'
  action approveRule(
